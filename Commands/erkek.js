...
const chat = message.guild.channels.cache.find(r => r.id === (Settings.Channels.GeneralChat))
  ...
  let isim = args[1]
  let yaş = args[2]
  let uyarıembed = new MessageEmbed().setColor(Settings.Colors.Red).setFooter("HyperFamilia").setTimestamp()
...
await db.push(`isimler.${user.id}`, {
  ...
  let embed = new MessageEmbed()
    ...
    ${Other.EmojiGeneral.Emoji7} **• Kayıt Edilen Kullanıcı:** ${user}
    ${Other.EmojiGeneral.Emoji8} **• Kayıt Eden Yetkili:** <@${message.author.id}>
    ${Other.EmojiGeneral.Emoji3} **• Verilen Roller:** <@&${Settings.Roles.BoyRole1}>, <@&${Settings.Roles.BoyRole2}>
    ${Other.EmojiGeneral.Emoji2} **• Yeni İsim:** \`${isim} | ${yaş}\`
`)
